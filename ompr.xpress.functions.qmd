...
# Import Libraries
library(ompr) |> suppressMessages() |> suppressWarnings()
library(xpress) |> suppressMessages() |> suppressWarnings()
...
unit_parameters <- unit_parameters |>
   mutate(PMAX = if_else(row_number()>= 100, PMAX, PMAX))
...
model <- model |>
  ...
  add_constraint(gen[p] == (ph_pmin*nrz_bin_one[p] + gen_stage_one[p] + 
                              ((nrz_max - nrz_min)*nrz_bin_two[p] + gen_stage_two[p])),
                 p = 1:per) |>
...
#### Build Row Equations From Parts
...
# Function To Retrieve Multiple Row Equations
get_row_equations <- function(prob, rows, dataframe = TRUE){
...
control = list(problem_name = 'Xpress Problem', 
               verbose = TRUE,
               inf_analysis = TRUE, 
               add_row_contraints = TRUE,
               MAXTIME = 1)
...
# Run Model
model_results <- model |> 
  solve_model(xpress_optimizer(MAXTIME = 1,
                               MIPRELSTOP = .0001))

```

#### Test Run Heur Solution

```{r}

# Extract Model Solution
model_heur_solution <- model_results$solution

# Pass In Heuristic
model_results_heur <- model |> 
  solve_model(xpress_optimizer(heur_sol = model_heur_solution))

```

#### Pass in Heur Solution

```{r}

# Data
bad_heur <- data.frame(Variable = rep(NA_character_,24),
                       Value = rep(NA_real_,24))

# Pass In Bad
model_results_bad_heur <- model |> 
  solve_model(xpress_optimizer(heur_sol = bad_heur))
...
# Check Data
model_results$additional_solver_output$inf_analysis$inf_equation
